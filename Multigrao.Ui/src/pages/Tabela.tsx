...
export default function Tabela() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Modal finalizar */}
      {modalFinalizar && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4" onClick={() => { setModalFinalizar(false); setSolicitante({ nome: '', telefone: '' }); }}>
          <div className="bg-white rounded-2xl border border-neutral-200 w-full max-w-md p-6 shadow-xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setModalFinalizar(false); setSolicitante({ nome: '', telefone: '' }); }} className="p-2 hover:bg-neutral-100 rounded-xl transition-colors"><X size={20} className="text-neutral-400" /></button>
            </div>
            <div className="space-y-3 mb-4">
              ...
              <div>
                ...
                <input value={solicitante.nome} onChange={e => setSolicitante({ ...solicitante, nome: e.target.value })} className="w-full bg-white border border-neutral-200 rounded-lg p-2.5 outline-none focus:border-amber-500 text-sm text-neutral-800 placeholder-neutral-400 mt-0.5" placeholder="Como podemos te chamar?" />
              </div>
              <div>
                <label className="text-sm font-medium text-neutral-700">Telefone / WhatsApp</label>
                <input value={solicitante.telefone} onChange={e => setSolicitante({ ...solicitante, telefone: e.target.value })} className="w-full bg-white border border-neutral-200 rounded-lg p-2.5 outline-none focus:border-amber-500 text-sm text-neutral-800 placeholder-neutral-400 mt-0.5" placeholder="(11) 99999-9999" />
              </div>
            </div>
            <button onClick={finalizarPedido} disabled={!solicitante.nome.trim() || enviando} className={`w-full py-2.5 rounded-xl font-semibold text-sm transition-colors ${solicitante.nome.trim() && !enviando ? 'bg-amber-600 text-white hover:bg-amber-500' : 'bg-neutral-200 text-neutral-400 cursor-not-allowed'}`}>
