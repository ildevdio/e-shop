...
interface PedidoConferencia {
  id: number;
  cliente: string;
  itens: number;
  valor: number;
  status: 'Aguardando' | 'Em Conferencia' | 'Conferido';
}
...
export default function Conferencia() {
  ...
  const [pedidoSelecionado, setPedidoSelecionado] = useState<PedidoConferencia | null>(null);
  const [itensConferencia, setItensConferencia] = useState<ItemConferencia[]>([]);

  const [pedidos, setPedidos] = useState<PedidoConferencia[]>([
    { id: 1003, cliente: 'Supermercado Nova Era', itens: 5, valor: 2500, status: 'Aguardando' },
    { id: 1004, cliente: 'Armazém Fit', itens: 3, valor: 800, status: 'Aguardando' },
    { id: 1005, cliente: 'Padaria Tradição', itens: 8, valor: 1200, status: 'Em Conferencia' },
    { id: 1006, cliente: 'Mercado São João', itens: 2, valor: 1450, status: 'Conferido' },
  ]);

  const iniciarConferencia = (pedido: PedidoConferencia) => {
    setPedidoSelecionado(pedido);
    setItensConferencia([
      { id: 1, nome: 'Castanha do Pará (1kg)', quantidade: 10, conferido: false },
      { id: 2, nome: 'Amendoim Torrado (5kg)', quantidade: 5, conferido: false },
      { id: 3, nome: 'Nozes (500g)', quantidade: 20, conferido: false },
      { id: 4, nome: 'Aveia em Flocos (10kg)', quantidade: 8, conferido: false },
      { id: 5, nome: 'Semente de Chia (2kg)', quantidade: 15, conferido: false },
    ]);
  ...
  const finalizarConferencia = () => {
    setPedidos(pedidos.map(p =>
      p.id === pedidoSelecionado?.id ? { ...p, status: 'Conferido' } : p
    ));
    setTelaAtiva('lista');
  ...
  // -----------------------------------------------------
  // TELA 1: LISTAGEM DE PEDIDOS PARA CONFERÊNCIA
  // -----------------------------------------------------
  if (telaAtiva === 'lista') {
    return (
        ...
        {/* KPIs */}
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm ring-1 ring-gray-100/50 flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-serif font-bold text-gray-900">{pedidos.filter(p => p.status === 'Aguardando').length}</h3>
            </div>
          </div>
          <div className="bg-white rounded-2xl p-5 shadow-sm ring-1 ring-gray-100/50 flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-serif font-bold text-gray-900">{pedidos.filter(p => p.status === 'Em Conferencia').length}</h3>
            </div>
          </div>
          <div className="bg-white rounded-2xl p-5 shadow-sm ring-1 ring-gray-100/50 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-[0.12em]">Conferidos</p>
              <h3 className="text-2xl font-serif font-bold text-gray-900">{pedidos.filter(p => p.status === 'Conferido').length}</h3>
            </div>
          </div>
        </div>
              ...
              <tbody>
                {pedidos.map(pedido => (
                  <tr key={pedido.id} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">{pedido.itens} itens</td>
                    <td className="px-6 py-4">R$ {pedido.valor.toLocaleString('pt-BR')}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      {pedido.status === 'Em Conferencia' && (
                        <button
                          onClick={() => iniciarConferencia(pedido)}
                          className="bg-blue-500 text-white px-4 py-2 rounded-xl text-xs font-semibold hover:bg-blue-600 transition-colors flex items-center gap-1.5 ml-auto shadow-sm"
                        >
                          <PackageCheck size={14} /> Continuar
                        </button>
                      )}
                      {pedido.status === 'Conferido' && (
                        <span className="text-black text-xs font-semibold flex items-center gap-1 justify-end">
                          <CheckCircle2 size={14} /> Pronto para Expedição
                        </span>
                      )}
                    </td>
                  </tr>
  ...
  // -----------------------------------------------------
  // TELA 2: CONFERÊNCIA DO PEDIDO (Checklist de Itens)
  // -----------------------------------------------------
      ...
      <div className="space-y-6 h-full flex flex-col">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-500 mt-1">{pedidoSelecionado.cliente} - {pedidoSelecionado.itens} itens</p>
          </div>
          <button
  ...
  // -----------------------------------------------------
  // TELA 3: LEITURA DO QR CODE (Scanner)
  // -----------------------------------------------------
      ...
      <div className="flex flex-col h-full bg-[#111111] -mx-8 -my-2 sm:m-0 text-white rounded-[2rem] sm:rounded-none overflow-hidden relative">
        <div className="absolute top-6 left-6 z-20">
          <button
            ...
            className="p-3 bg-white/10 rounded-full backdrop-blur-md active:bg-white/20 transition-colors"
          >
            <ArrowLeft size={24} />
          </button>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          ...
          <div className="w-64 h-64 border-2 border-black/50 border-dashed rounded-3xl relative mb-12 flex items-center justify-center bg-black/5 shadow-[0_0_50px_rgba(16,185,129,0.1)]">
          ...
          <button
            onClick={confirmarQR}
            className="w-full max-w-[320px] bg-gradient-to-r from-gray-900 to-gray-700 text-white font-bold text-lg py-4 rounded-2xl active:scale-[0.98] transition-all shadow-lg shadow-black/30 tracking-wide"
          >
            SIMULAR LEITURA
          </button>
        </div>
  ...
  // -----------------------------------------------------
  // TELA 4: RESULTADO DA CONFERÊNCIA
  // -----------------------------------------------------
      ...
      <div className="flex flex-col h-full bg-[#111111] -mx-8 -my-2 sm:m-0 text-white rounded-[2rem] sm:rounded-none overflow-hidden">
        ...
        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar flex flex-col items-center justify-center text-center">
          ...
          <p className="text-gray-400 mb-8 max-w-sm">
            Pedido #{pedidoSelecionado?.id} - {pedidoSelecionado?.cliente} foi verificado e está apto para expedição.
          </p>
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    'Aguardando': 'bg-amber-50 text-amber-700 ring-amber-500/20',
                    'Em Conferencia': 'bg-blue-50 text-blue-700 ring-blue-500/20',
