...
import { BarChart3, Plus, Search, Edit3, Trash2 } from 'lucide-react';
import { Link } from 'react-router-dom';
import { ArrowLeft } from 'lucide-react';

export default function EmpresaEnquetes() {
  const [enquetes, setEnquetes] = useState([
    { id: 1, titulo: 'Qual melhor horário de almoço?', opcoes: [{ texto: '11:30', votos: 5 }, { texto: '12:00', votos: 12 }, { texto: '12:30', votos: 8 }], totalVotos: 25, status: 'Ativa', votou: false },
    { id: 2, titulo: 'Nova pausa café?', opcoes: [{ texto: 'Sim, 15min', votos: 20 }, { texto: 'Não preciso', votos: 3 }], totalVotos: 23, status: 'Encerrada', votou: true },
  ]);
  ...
  const votar = (enqueteId: number, opcaoIdx: number) => {
    setEnquetes(enquetes.map(e => {
      if (e.id === enqueteId && !e.votou && e.status === 'Ativa') {
        const novasOpcoes = e.opcoes.map((o, i) => i === opcaoIdx ? { ...o, votos: o.votos + 1 } : o);
        return { ...e, opcoes: novasOpcoes, totalVotos: e.totalVotos + 1, votou: true };
      }
      return e;
    }));
  };
      ...
      <div className="flex-1 bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden flex flex-col">
        <div className="p-6 flex justify-between items-center border-b border-gray-100">
          <div className="relative">
            ...
            <input type="text" placeholder="Buscar enquetes..."               className="pl-10 pr-4 py-2.5 bg-gray-50/50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-black/20 focus:border-black text-sm flex-1 min-w-0 transition-all" />
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {enquetes.map(enquete => (
            <div key={enquete.id} className={`p-5 rounded-2xl border-2 transition-all ${enquete.status === 'Ativa' ? 'bg-gray-50/50 border-gray-200 hover:border-gray-300' : 'bg-gray-100/50 border-gray-200 opacity-80'}`}>
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-gray-400">{enquete.totalVotos} votos</span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] px-3 py-1 rounded-full font-bold uppercase tracking-wider ${enquete.status === 'Ativa' ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}>{enquete.status}</span>
                  <button className="text-gray-400 hover:text-gray-600 transition-colors"><Edit3 size={16} /></button>
                  <button onClick={() => setEnquetes(enquetes.filter(e => e.id !== enquete.id))} className="text-gray-400 hover:text-red-500 transition-colors"><Trash2 size={16} /></button>
                </div>
              </div>
              ...
              <div className="space-y-3 ml-[52px]">
                {enquete.opcoes.map((opcao, idx) => {
                  ...
                  return (
                    <button key={idx} onClick={() => votar(enquete.id, idx)} disabled={enquete.votou || enquete.status === 'Encerrada'} className={`w-full text-left p-3 rounded-xl border transition-all ${!enquete.votou && enquete.status === 'Ativa' ? 'border-gray-200 hover:border-gray-400 hover:bg-gray-50 cursor-pointer' : 'border-gray-200 cursor-default'} relative overflow-hidden`}>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
                ...
                <div className="space-y-2">
                  {novaEnquete.opcoes.map((op, idx) => (
                    <div key={idx} className="flex gap-2">
                      ...
                      {novaEnquete.opcoes.length > 2 && <button onClick={() => removerOpcao(idx)} className="text-gray-400 hover:text-red-500 transition-colors"><Trash2 size={18} /></button>}
                    </div>
                  ))}
                </div>
            ...
            <div className="flex gap-3 justify-end mt-6">
              ...
              <button onClick={() => {
                if (!novaEnquete.titulo.trim() || novaEnquete.opcoes.some(o => !o.trim())) return;
                setEnquetes([{ id: Date.now(), titulo: novaEnquete.titulo, opcoes: novaEnquete.opcoes.filter(o => o.trim()).map(o => ({ texto: o, votos: 0 })), totalVotos: 0, status: 'Ativa', votou: false }, ...enquetes]);
